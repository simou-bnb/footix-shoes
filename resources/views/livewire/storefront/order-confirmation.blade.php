@php $pixelData = base64_encode(json_encode(['value'=>(float)$order->total,'currency'=>'DZD','content_type'=>'product','content_ids'=>$order->items->pluck('product_variant_id')->map(fn($id)=>(string)$id)->values()->all(),'num_items'=>(int)$order->items->sum('quantity')])); @endphp
