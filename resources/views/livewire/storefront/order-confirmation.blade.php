@php $pixelData = json_encode(['value'=>(float)$order->total,'currency'=>'DZD','content_type'=>'product','content_ids'=>$order->items->pluck('product_variant_id')->map(fn($id)=>(string)$id)->values()->all(),'num_items'=>(int)$order->items->sum('quantity')]); @endphp
<div class="max-w-2xl mx-auto px-4 sm:px-6 py-12 sm:py-20 text-center"
    x-data="{ fired: false }"
    x-init="
        if (!fired) {
            console.log('Triggering Purchase via Alpine', {{ $pixelData }});
            if (typeof fbq !== 'undefined') fbq('track', 'Purchase', {{ $pixelData }});
            fired = true;
        }
    ">
