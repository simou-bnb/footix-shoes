<div
    ...
    x-init="initCheckoutPixel()"
    class="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
    <h1 class="text-3xl font-extrabold uppercase tracking-tight mb-8 border-b border-black pb-6">Commander</h1>
    ...
    @else
        ...
        <form wire:submit="placeOrder" class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div class="lg:col-span-2 space-y-5">
                <div>
                    <label class="block text-xs uppercase tracking-wide font-medium mb-1">Nom complet</label>
                ...
                <div>
                    <label class="block text-xs uppercase tracking-wide font-medium mb-1">Téléphone</label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-xs uppercase tracking-wide font-medium mb-1">Wilaya</label>
                    ...
                    <div>
                        <label class="block text-xs uppercase tracking-wide font-medium mb-1">Commune</label>
                ...
                @if ($this->selectedWilaya)
                    <div>
                        <label class="block text-xs uppercase tracking-wide font-medium mb-2">Livraison</label>
                ...
                @if ($deliveryType === 'home')
                    <div>
                        <label class="block text-xs uppercase tracking-wide font-medium mb-1">Adresse</label>
            ...
            <div class="lg:col-span-1">
                <div class="border border-black p-5 sticky top-24">
                    <h2 class="text-sm font-bold uppercase tracking-wide mb-4">Récapitulatif</h2>

                    <div class="space-y-3 mb-4 max-h-64 overflow-y-auto">
                        @foreach ($items as $item)
                            <div class="flex justify-between text-sm">
                                <span class="text-black/70">{{ $item->variant->product->name }} ({{ $item->variant->label }}) &times;{{ $item->quantity }}</span>
                                <span class="shrink-0 ml-2">{{ number_format($item->lineTotal, 0, ',', ' ') }} DA</span>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <p class="text-xs text-black/50 mt-3 text-center">Paiement en espèces à la livraison.</p>
                </div>
            </div>
        </form>
    @endif
